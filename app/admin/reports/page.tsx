  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex h-12 items-center gap-2 rounded-2xl border border-slate-200 bg-white/75 px-4 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.06]">
        ...
        <input placeholder="Search reports" className="w-full bg-transparent text-sm outline-none" />
      </div>
      <AdminEmptyState icon={Flag} title="No reports are exposed to admin yet" description="Reported posts, users, and moderation outcomes will appear here when the backend provides them." />
    </div>
  );
}
